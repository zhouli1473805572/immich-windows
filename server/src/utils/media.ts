...
export class BaseHWConfig extends BaseConfig implements VideoCodecHWConfig {
  ...
  validateDevices(devices: string[]) {
    if (devices.length === 0) {
      throw new Error('No /dev/dri devices found. Make sure the expected hardware acceleration device is available.');
    }
  ...
  getDevice({ dri }: VideoInterfaces) {
    ...
    if (!dri.includes(deviceName)) {
      throw new Error(`Device '${deviceName}' does not exist. Make sure the expected hardware acceleration device is available.`);
    }
